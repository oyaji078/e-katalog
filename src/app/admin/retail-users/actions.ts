...
export async function approveRetailUserAction(
  ...
  const tokenResult = await db.$transaction(async (tx) => {
    ...
    if (existingToken) {
      return { token: existingToken.tokenPreview, reused: true };
    }
    ...
    await tx.retailToken.create({
      data: {
        ...
        notes: `Approval token for retail applicant ${user.name} (${user.email})`,
      },
    });

    return { token: generated.token, reused: false };
  });
