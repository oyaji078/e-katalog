...
import { deletePromoBannerAction } from "./actions";
...
export default function DeleteBannerButton({ id }: { id: string }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || "Banner promo gagal dihapus.");
        return;
      }
      ...
      setError("Banner promo gagal dihapus. Silakan coba lagi.");
    } finally {
      setIsLoading(false);
    }
  }
    ...
    <div className="relative">
      ...
      {isConfirming ? (
        <div className="absolute right-0 z-20 mt-2 w-72 rounded-lg border border-brand-border bg-white p-4 text-left shadow-xl">
          <h3 className="text-sm font-bold text-brand-text">Hapus Banner Promo?</h3>
          <p className="mt-2 text-xs leading-5 text-brand-muted">
            Banner promo akan dihapus dan tidak tampil lagi di halaman public.
          </p>
