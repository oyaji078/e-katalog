...
export function isValidCategoryLogoUrl(value: string) {
  const trimmed = value.trim();
  if (!/^\/uploads\/categories\/category-[a-f0-9]{8}\.webp$/.test(trimmed)) {
    return false;
  }
...
export function safeCategoryLogoSrc(value: string | null | undefined) {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  return isValidCategoryLogoUrl(trimmed) ? trimmed : null;
}
